...
fn record_history_item(log: &mut Buffer, item: String) {
    ...
    if let Err(error) = state::save_state(log) {
        eprintln!("Failed to save clipboard state: {}", error);
    }
}
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if args.len() > 1 && args[1] == "--menu" {
        ...
        match state::load_state() {
            ...
            Err(error) => {
                eprintln!("Failed to load clipboard state: {}", error);
            }
        }
    ...
    if let Err(error) = storage::cleanup_orphaned_images(&clipboard_log.get_items()) {
        eprintln!("Failed to clean orphaned images: {}", error);
    }
            ...
            match WaylandClipboard::new() {
                Ok(mut clipboard) => {
                    ...
                    if let Ok(image_bytes) = clipboard.read_image(image_mime) {
                        ...
                        let file_path = match storage::image_path(ext) {
                            Ok(path) => path,
                            Err(error) => {
                                eprintln!("Failed to create image path: {}", error);
                                continue;
                            }
                        };
                        if fs::write(&file_path, &image_bytes).is_ok() {
                            ...
                            let formatted_image = format!(
                                "{}{}",
                                storage::IMAGE_PREFIX,
                                file_path.display()
                            );
